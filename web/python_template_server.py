...
def get_task_result():
    ...
    user_name = param_dict['user_name']
    res_list = dbm.query(sql_dict["last_task"].format(user_name))
    if len(res_list) == 0:
        return Response(json.dumps({'msg': '您没有进行中的任务', 'status': 2, 'data': {}}, ensure_ascii=False),
                        mimetype='application/json',
                        status=200)
    ...
    if res_list[0][4] == 2:
        return Response(
            json.dumps({'msg': '任务完成', 'status': 0, 'data': {'img': res_list[0][3]}}, ensure_ascii=False),
            mimetype='application/json',
            status=200)
